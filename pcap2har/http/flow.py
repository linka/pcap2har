...
class Flow(object):
    ...
    def __init__(self, tcpflow, drop_response_bodies):
        '''
        tcpflow = tcp.Flow
		drop_response_bodies = if True, response bodies are dropped
        '''
...
def gather_messages(MessageClass, tcpdir, spec_args=None):
	...
	* spec_args = list of specialarguments, that will be passed to the message
    Returns:
    [MessageClass]
...
def parse_streams(request_stream, response_stream, drop_response_bodies):
	...
	drop_response_bodies = drop response bodies if True
    Returns:
